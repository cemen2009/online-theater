...
class CSVDatabaseSeeder:
    """
    Class responsible for seeding the database with data from a CSV file.
    ...
    async def is_db_populated(self) -> bool:
        """
        Check if movie database already contains records.

        :return: True if database contains at least one record, False otherwise.
        :rtype: bool
        """
    ...
    async def _preprocess_csv(self) -> pd.DataFrame:
        """
        Load and preprocess the CSV file before inserting data into database.
